...
Note: Kolla-ansible does not deploy Ceph itself. Use cephadm, Rook, or a standalone Ceph deployment. Then configure
kolla-ansible to use external Ceph by providing the ceph.conf and keyrings in the `config/` directory.
...
Use NFS when you have an existing NFS server or NAS appliance and want to avoid the complexity of Ceph. Supports live
migration since storage is shared.
...
**Key rule:** Choose Ceph for production from the start if you can. Migrating storage backends later is possible but
involves per-volume data migration and a maintenance window.
