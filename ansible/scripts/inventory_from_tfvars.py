...
def build_inventory() -> dict:
    ...
    hosts: list[str] = []
    k3s_server_hosts: list[str] = []
    k3s_agent_hosts: list[str] = []
    ...
    for index, source_host_name in enumerate(source_hosts, start=1):
        ...
        raw_cluster_role = labels.get("role") or server_config.get("role")
        cluster_role = (
            clean_wrapped_quotes(str(raw_cluster_role)).strip().lower()
            if raw_cluster_role is not None
            else ""
        )
        ...
        if cluster_role == "server":
            k3s_server_hosts.append(host_alias)
        elif cluster_role == "agent":
            k3s_agent_hosts.append(host_alias)
        hostvars[host_alias] = {
